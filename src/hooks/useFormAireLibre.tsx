...
export default function useFormAireLibre() {
    ...
    function procesarString(input: string): { marca: string; precio: number } {
        ...
        if (match) {
            ...
            const precioRaw = match[2];

            // Procesar el precio: eliminar puntos, reemplazar coma por punto
            const precioNumerico = parseFloat(precioRaw.replace(/\./g, "").replace(",", "."));
    ...
    const normalizeInfo = (data: any[]) => {
        ...
        filter_data_1.map((n, i) => {
            if (actual_1 === undefined) {
                ...
                if (typeof codigo === "undefined" || typeof nombre === "undefined") return;
